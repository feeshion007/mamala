...
define(function(require,exports,module){
			...
			ShopSelector.prototype = {
				...
				initialize :function (options) {
					...
					this.defaultTip = $.i18n.prop('common_shop_select_city_name');
				...
				formatData: function(data){
					...
					for(var i=0;i<data.length;i++){
						...
						for(var provinceName in provinceInfo){
							//判断当前省属于哪个字母开头
							var newProvinceName = provinceName.replace(/\s/ig,'');
							var matchProvince = regExProvince.exec(newProvinceName);
							if(!matchProvince){
							...
							for(var j = 0;j<citys.length;j++){
								...
								for(var cityName in city){									
									...
									var newCityName = cityName.replace(/\s/ig,'');
									var matchCity = regExCity.exec(newCityName);
									if(!matchCity){
			...
			// 获取当前城市
				...
				createWarp:function(){
					...
					$(titleDiv).append('<p class="tip">'+$.i18n.prop('common_shop_select_shop_list')+'</p>');
					...
					$(searchDiv).append('<input type="button" id = "byProvince" value="'+$.i18n.prop('common_shop_select_by_province')+'">');
					$(searchDiv).append('<input type="button" id = "byCity" value="'+$.i18n.prop('common_shop_select_by_city')+'">');
					$(searchDiv).append('<input type="text" class="cityinput" id="cityInput" value='+that.defaultTip+'>');
					$(searchDiv).append('<input type="button" id = "searchButton" value="'+$.i18n.prop('common_search')+'">');
					...
					for(var i = 0;i<sortKey.length;i++){
						...
						for(var k = 0; k< subData.length ; k++){
							...
							for(var districtName in district){
								...
								var match = this.regExProvince.exec(districtName)||this.regEx.exec(districtName)||["",$.i18n.prop('common_shop_select_illegal_name')];
			   ...
			   /* *
				...
				inputEvent: function(){
					...
					$("#searchButton").click(function(){
						...
						var cityName = that.input.shop || $.i18n.prop('common_shop_select_shop_info_none');
				...
				createUl:function () {
					...
					if (value !== '') {
						...
						if (searchResult.length == 0) {
							this.isEmpty = true;
							str = '<li class="empty">'+$.i18n.prop('common_shop_select_no_data_sorry')+'"<em>' + value + '</em>"</li>';
							searchResult.push(str);
				...
				/* 城市HTML模板 */
				_template : [
					'<p class="tip">'+$.i18n.prop('common_shop_select_city_list')+'</p>',
					'<ul>',
					'<li class="on">'+$.i18n.prop('common_shop_select_city_hot')+'</li>',
					'<li>ABCDEFGH</li>',
					'<li>IJKLMNOP</li>',
					'<li>QRSTUVWXYZ</li>',
					'</ul>'
