...
rui.Grid.prototype = {
	...
	update : function(gridData) {
		...
		if(gridData) {
			...
			//刷新数据后,还原排序状态
			$(".rui-grid-header-th").removeClass("rui-grid-header-des");
			$(".rui-grid-header-th").removeClass("rui-grid-header-asc");
		...
		//增加title ，实现表头的tooltip功能
			...
			if(!this._isNull(orderBy)) {
				...
				if(this.orderNeedTitle) {
					if(isZh) {
						thCell.title = $.i18n.prop('common_click_by') + tHeadName + $.i18n.prop('common_order');
					} else {
	...
	_sortColumn : function(cell) {
		...
		}else if(cell.className.indexOf("rui-grid-header-asc") >= 0){
	...
	_createTableData : function() {
		...
		if(!this.gridData || this.gridData.length <= 0) {
			...
			if(window.language && window.language == "en") {
				...
				this._defaultNoRecordMsg = "<b>"+$.i18n.prop('common_no_record')+"</b>";
			}
	...
	_initPageWidget : function() {
		...
		if(this.isPage && this._isNull(this.pageWidget)) {
			...
			this.pageWidget = new ruiPage(this.pageWidgetOptions);
